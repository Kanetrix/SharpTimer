            ...
            try
            {
                if (File.Exists(playerReplaysPath))
                {
                    ...
                    else
                    {
                        ...
                        Server.NextFrame(() => player.PrintToChat(msgPrefix + $" Zdá sa, že požadované prehratie je poškodené"));
                    }
                }
                else
                {
                    SharpTimerError($"File does not exist: {playerReplaysPath}");
                    Server.NextFrame(() => player.PrintToChat(msgPrefix + $" Požadované prehratie neexistuje"));
                }
            }
            catch (Exception ex)
            {
