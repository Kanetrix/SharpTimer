...
namespace SharpTimer
{
    public partial class SharpTimer
    {
        public void PrintAllEnabledCommands(CCSPlayerController player)
        {
            ...
            player.PrintToChat($"{msgPrefix}Dostupné príkazy:");

            if (respawnEnabled) player.PrintToChat($"{msgPrefix}!r (css_r) - Obnoví vás");
            if (respawnEnabled && bonusRespawnPoses.Count != 0) player.PrintToChat($"{msgPrefix}!rb <#> / !b <#> (css_rb / css_b) - Obnoví vás na bonus");
            if (topEnabled) player.PrintToChat($"{msgPrefix}!top (css_top) - Uvádza 10 najlepších záznamov na tejto mape");
            if (topEnabled && bonusRespawnPoses.Count != 0) player.PrintToChat($"{msgPrefix}!topbonus <#> (css_topbonus) - Uvádza 10 najlepších záznamov bonusu");
            if (rankEnabled) player.PrintToChat($"{msgPrefix}!rank (css_rank) - Zobrazuje vašu aktuálnu hodnosť a pb");
            if (globalRanksEnabled) player.PrintToChat($"{msgPrefix}!points (css_points) - Vytlačí 10 najlepších bodov");
            if (goToEnabled) player.PrintToChat($"{msgPrefix}!goto <name> (css_goto) - Teleportuje vás k hráčovi");
            if (stageTriggerPoses.Count != 0) player.PrintToChat($"{msgPrefix}!stage <#> (css_stage) - Teleportuje vás na javisko");

            if (cpEnabled)
            {
                player.PrintToChat($"{msgPrefix}{(currentMapName!.Contains("surf_") ? "!saveloc (css_saveloc) - Uloží miesto" : "!cp (css_cp) - Nastaví kontrolný bod")}");
                player.PrintToChat($"{msgPrefix}{(currentMapName!.Contains("surf_") ? "!loadloc (css_loadloc) - Teleportuje vás na posledné miesto" : "!tp (css_tp) - Teleportuje vás k poslednému kontrolnému bodu")}");
                player.PrintToChat($"{msgPrefix}{(currentMapName!.Contains("surf_") ? "!prevloc (css_prevloc) - Teleportuje vás o jednu polohu späť" : "!prevcp (css_prevcp) - Teleportuje vás o jeden kontrolný bod späť")}");
                player.PrintToChat($"{msgPrefix}{(currentMapName!.Contains("surf_") ? "!nextloc (css_nextloc) - Teleportuje vás o jednu polohu dopredu" : "!nextcp (css_nextcp) - Teleportuje vás o jeden kontrolný bod dopredu")}");
            }

            if (jumpStatsEnabled) player.PrintToChat($"{msgPrefix}!jumpstats (css_jumpstats) - Prepnutie JumpStats");
        }
        ...
        public async Task PrintMapTimeToChat(CCSPlayerController player, string steamID, string playerName, int oldticks, int newticks, int bonusX = 0, int timesFinished = 0)
        {
            ...
            Server.NextFrame(() =>
            {
                if (IsAllowedPlayer(player) && timesFinished > maxGlobalFreePoints && globalRanksFreePointsEnabled == true && oldticks < newticks)
                    player.PrintToChat(msgPrefix + $"{ChatColors.White} Dosiahli ste maximálny počet bezplatných odmien vo výške {primaryChatColor}{maxGlobalFreePoints}{ChatColors.White}!");

                if (newSR)
                {
                    Server.PrintToChatAll(msgPrefix + $"{primaryChatColor}{playerName} {ChatColors.White}zvládol nový {(bonusX != 0 ? $"Bonus {bonusX} SR!" : "SR!")}");
                ...
                {
                    Server.PrintToChatAll(msgPrefix + $"{primaryChatColor}{playerName} {ChatColors.White}zvládol nový {(bonusX != 0 ? $"Bonus {bonusX} PB!" : "Map PB!")}");
                ...
                {
                    Server.PrintToChatAll(msgPrefix + $"{primaryChatColor}{playerName} {ChatColors.White}dokončil {(bonusX != 0 ? $"Bonus {bonusX}!" : "Map!")}");
                ...
                if (useMySQL != false || bonusX != 0)
                    Server.PrintToChatAll(msgPrefix + $"{(bonusX != 0 ? $"" : $"Hodnosť: [{primaryChatColor}{ranking}{ChatColors.White}] ")}{(timesFinished != 0 && useMySQL == true ? $"Časy dokončené: [{primaryChatColor}{timesFinished}{ChatColors.White}]" : "")}");

                Server.PrintToChatAll(msgPrefix + $"Čas: [{primaryChatColor}{newTime}{ChatColors.White}] {timeDifference}");
        ...
        public char GetRankColorForChat(CCSPlayerController player)
        {
            try
            {
                if (playerTimers.TryGetValue(player.Slot, out PlayerTimerInfo? playerTimer))
                {
                    ...
                    if (playerTimer.CachedRank.Contains("Nehodnotený"))
